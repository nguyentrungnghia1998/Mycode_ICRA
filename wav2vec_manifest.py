...
def main(args):
    ...
    with open(os.path.join(args.dest, "train.tsv"), "w") as train_f:
        ...
        all_files = glob.iglob(search_path, recursive = True)
        ...
        for fname in tqdm(all_files):
            ...
            sample_rate = soundfile.info(fname).samplerate
            duration = int(frames/sample_rate*1000)
            dest = train_f if rand.random() > args.valid_percent else valid_f
            print(
                "{}\t{}".format(os.path.relpath(file_path, dir_path), duration), file=dest
            )
    if valid_f is not None:
        valid_f.close()
